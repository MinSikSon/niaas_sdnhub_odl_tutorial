...
public class Niaas_Change_Path {
	...
	public static void change_cloud_to_fog(byte[] payload, String srcIp_cloud, String dstIp_fog, String srcMac_cloud, String dstMac_fog){
		int LOG_TEST = 0; // SMS
		final Logger LOG = LoggerFactory.getLogger(Niaas_Change_Path.class);
		...
		// ip
		byte[] byteArray_srcIp = Niaas_Parser_IpAddr.get_byteArray_SrcIp(payload); // & display Addr 
		byte[] byteArray_dstIp = Niaas_Parser_IpAddr.get_byteArray_DstIp(payload); // & display Addr
		String string_srcIp = Niaas_Parser_IpAddr.ipAddr_byteArray_to_stringIp(byteArray_srcIp);
		String string_dstIp = Niaas_Parser_IpAddr.ipAddr_byteArray_to_stringIp(byteArray_dstIp);
...
//			return;
		...
		if(string_dstIp.equals(srcIp_cloud)){ // 10.0.0.3
			...
			tmpDstIP = Niaas_Parser_IpAddr.ipAddr_intArray_to_byteArray(Niaas_Parser_IpAddr.ipAddr_stringIp_to_intArray(dstIp_fog));
			//string_to_byteArray;
			if(dstMac_fog == null) {
				if(LOG_TEST == 1) LOG.debug("-------------------------change_cloud_to_fog-------------------------");
				return;
			...
			tmpDstMac = Niaas_Parser_MacAddr.macAddr_intArray_to_byteArray(Niaas_Parser_MacAddr.macAddr_stringMac_to_intArray(dstMac_fog)); // VV
...
//				tmpIpChecksum = SMS_Parser_IpAddr.ipv4_Calculate_Checksum(byteIpHeader);
				tmpIpChecksum = Niaas_Checksum.ipv4_Calculate_Checksum(byteIpHeader);
...
//				tmpTcpChecksum = SMS_Parser_IpAddr.tcp_Calculate_Checksum(payload, srcIp, dstIp_fog);
				tmpTcpChecksum = Niaas_Checksum.tcp_Calculate_Checksum(payload, string_srcIp, dstIp_fog);
...
//			return;
		...
		if(string_srcIp.equals(dstIp_fog)){ // 10.0.0.10
			...
			tmpSrcIP = Niaas_Parser_IpAddr.ipAddr_intArray_to_byteArray(Niaas_Parser_IpAddr.ipAddr_stringIp_to_intArray(srcIp_cloud));
			//string_to_byteArray;
			if(srcMac_cloud == null) return;
			if(LOG_TEST == 1) LOG.debug("[srcMac_fog] {}",srcMac_cloud);
			tmpSrcMac = Niaas_Parser_MacAddr.macAddr_intArray_to_byteArray(Niaas_Parser_MacAddr.macAddr_stringMac_to_intArray(srcMac_cloud)); // VV
...
//				tmpTcpChecksum = SMS_Parser_IpAddr.tcp_Calculate_Checksum(payload, srcIp_cloud, dstIp);
				tmpTcpChecksum = Niaas_Checksum.tcp_Calculate_Checksum(payload, srcIp_cloud, string_dstIp);
				payload[50] = tmpTcpChecksum[0];
				payload[51] = tmpTcpChecksum[1];
				if(LOG_TEST == 1) LOG.debug("[after ] Tcpchecksum: {} {}", payload[DST_IP_END_POSITION+17], payload[DST_IP_END_POSITION+18]);
			}
